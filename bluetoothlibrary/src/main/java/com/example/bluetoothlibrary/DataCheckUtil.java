...
public class DataCheckUtil extends Toast {
    ...
    public static String stringFormat(String url) {
        ...
        if (url.contains("\\")) {
            ...
            temp = m.replaceAll("%5C");
        }
        // �ж�+
        if (url.contains("+")) {
            Pattern p = Pattern.compile("\\+");
            Matcher m = p.matcher(temp);
            temp = m.replaceAll("%2B");
