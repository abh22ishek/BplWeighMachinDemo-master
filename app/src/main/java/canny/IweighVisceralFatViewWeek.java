...
public class IweighVisceralFatViewWeek extends View {
    ...
    List<RecordDetailWeighMachine> listVFat;

    List<String> vFatList;
    ...
    public IweighVisceralFatViewWeek(Context context) {
        super(context);
        init();
    }

    public IweighVisceralFatViewWeek(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(vFatList.size()>0)
        {
            p1y1=(heightScaleRatio-(Float.parseFloat(vFatList.get(0))))*height_scale;
        }
        ...
        for(int i = 0; i<vFatList.size(); i++)
        {
            float y_points=(heightScaleRatio-(Float.parseFloat(vFatList.get(i))));
            float px_y=y_points*height_scale;// current y pointts

            canvas.drawCircle(mpX,px_y,7,paint);
            canvas.drawText("( "+vFatList.get(i)+" )",mpX,px_y,mPaint);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if(listVFat.size()<=3){
            setMeasuredDimension((6*listVFat.size()*pixels_per_unit)+pixels_per_unit,
                    6*pixels_per_unit);
        }else{
            setMeasuredDimension((2*listVFat.size()*pixels_per_unit)+pixels_per_unit,
                    6*pixels_per_unit);
        }
    ...
    public List<String> setPlotPoints(List<String> points){
        vFatList=points;
        Logger.log(Level.DEBUG,"Custom View ","**Week record size**="+vFatList.size());
        return vFatList;
    }
}
