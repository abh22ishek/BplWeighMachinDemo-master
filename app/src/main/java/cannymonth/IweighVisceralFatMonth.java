...
public class IweighVisceralFatMonth extends View {
    ...
    List<RecordDetailWeighMachine> listVisceralFat;

    List<String> visceralFatList;
    ...
    public IweighVisceralFatMonth(Context context) {
        super(context);
        init();
    }

    public IweighVisceralFatMonth(Context context,  AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float p1y1=0;
        if(visceralFatList.size()>0)
        {
            p1y1=(heightScaleRatio-(Float.parseFloat(visceralFatList.get(0))))*height_scale;
        }
        ...
        for(int i = 0; i<visceralFatList.size(); i++)
        {
            float y_points=(heightScaleRatio-(Float.parseFloat(visceralFatList.get(i))));
            float px_y=y_points*height_scale;// current y pointts

            canvas.drawCircle(mpX,px_y,7,paint);
            canvas.drawText("( "+visceralFatList.get(i)+" )",mpX,px_y,mPaint);
    ...
    public List<String> setPlotPoints(List<String> points){
        visceralFatList=points;
        Logger.log(Level.DEBUG,"Custom View ","**Month record size**="+visceralFatList.size());

        return visceralFatList;
    }
}
