...
public class MyCustomChartHRDay extends View{
    ...
    public MyCustomChartHRDay(Context context) {
        super(context);
    ...
    public MyCustomChartHRDay(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        @SuppressLint("DrawAllocation") List<String> mTime=new ArrayList<>();
        ...
        canvas.drawText("150",0,getHeight()/4+20,mPaint);
        canvas.drawText("50",0,3*getHeight()/4,mPaint);*/
        ...
        p.setColor(Color.parseColor("#7CBC50"));

        // draw circle points
        float mpX=0;
        for(int i=0;i<Integer.parseInt(UserTestReportGraphActivity.total_time);i++)
        {
            float y_points=(200-(Integer.parseInt(TestReportDayFragment.mpr_list[i])));
        ...
        // first point
        float p1x1=0;
        float p1y1=(200-(Integer.parseInt(TestReportDayFragment.mpr_list[0])))*height_scale;
